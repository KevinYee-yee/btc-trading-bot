...
STOP_PCT        = float(os.environ.get("STOP_PCT", "1.5"))        # 交易所止損單距離%（T趨勢腿=8）
EMA_FAST        = float(os.environ.get("EMA_FAST", "13"))
EMA_SLOW        = float(os.environ.get("EMA_SLOW", "48"))
...
def _live_sync_position(portfolio):
    ...
    if exchange_qty > threshold and json_qty <= threshold:
        ...
        if portfolio.get("live_algo_id"):
            ...
            stop_px = portfolio.get("live_stop_px") or round(entry * (1 - STOP_PCT / 100), 4)
...
def run():
    ...
    if LIVE_TRADE or os.environ.get("PAPER_GATE", "").lower() == "true":
